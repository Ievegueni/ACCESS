...
const ZH = {
  ...
  'ord.expirada': '已过期',
  'ord.aVerificar': '核查中',
  'ord.m.naoRecolhida': '没有设备取走该指令：未执行任何操作',
  'ord.m.semConfirmacao': '已执行但未收到确认短信：我方正在核查，请勿重发',
  'ord.m.falhaOperador': '运营商回复中没有“成功”：我方正在核查，请勿重发',
  'ord.m.verificacaoManual': '结果已由我方对照账单确认',
  'ord.m.operadorIndisponivel': '运营商服务不可用，重试未成功：未执行任何操作',
  'ord.a.aceite': '已送达',
  'ord.a.aTentar': '重试中',
  'ord.a.recusado': '未被接受',
  'ord.a.porEnviar': '待发送',
  ...
  'dev.api.c.campos': '该操作所需的数值：<code>valor</code>、<code>iban</code> …… 最多 10 个。<code>valor</code> 须为整数宽扎：<code>"500"</code>。<code>"500.00"</code> 会被接受并保存为 <code>"500"</code>；带分（<code>"500.50"</code>）返回 <code>400</code>',
  'dev.api.c.numero': '指定由哪台设备执行。不填则贵方任一台设备都可以',
  'dev.api.h.estado': '查询状态',
  'dev.api.est': '状态',
  'dev.api.s.emCurso': '尚未结束：排队中、执行中，或正在由我方核查',
  'dev.api.s.emCurso.f': '等待通知。不要为同一笔提现再创建指令。',
  'dev.api.s.sucesso': '转账已完成。<code>tid</code> 为运营商的交易号',
  'dev.api.s.sucesso.f': '将该笔提现标记为已支付。',
  'dev.api.s.falha': '转账<b>未</b>完成。没有任何资金转出',
  'dev.api.s.falha.f': '将该笔提现标记为失败。可以用新的 <code>ref</code> 重发。',
  'dev.api.estados.nota': '只有 <code>sucesso</code> 和 <code>falha</code> 是最终状态，之后不会再变。我方从不发送不确定的结果：运营商未能明确确认时，我方会先核查再结束指令，在此之前指令保持 <code>em_curso</code>。',
  'dev.api.motivo.nota': '<code>motivo</code> 仅供参考，只出现在最终状态中。贵方的逻辑应以 <code>estado</code> 为准。',
  'dev.api.m.nulo': '已由运营商确认',
  'dev.api.m.verificacaoManual': '运营商未能明确确认，我方已对照账户账单核实',
  'dev.api.c.notify': '贵方的 <code>https://</code> 地址。指令以 <code>sucesso</code> 或 <code>falha</code> 结束时，我方向该地址发送 <code>POST</code>，内容与状态查询的返回相同',
  'dev.api.r5.d0': '每条指令只通知一次，且只在其以 <code>sucesso</code> 或 <code>falha</code> 结束时通知。大多数指令几秒内结束；需要核查的指令可能耗时更长（最多数小时）。',
  'dev.api.teste.d': '如需在不转账的情况下测试贵方接收端，可在此发送一条测试通知：使用贵方密钥签名，<code>ref</code> 为 <code>TESTE-CALLBACK</code>。',
  'dev.api.teste.enviar': '发送测试通知',
  'dev.api.teste.aEnviar': '发送中……',
  'dev.api.teste.semResposta': '无响应（网络、DNS 或超过 10 秒）',
  'dev.api.teste.aceite': '已接受：正式通知将被视为已送达。',
  'dev.api.teste.recusado': '未被接受：正式通知将重试最多 8 次。',
  'dev.api.r3.t': '指令处于 <code>em_curso</code> 时，切勿创建新的 <code>ref</code>',
  'dev.api.r3.d': '这是同一笔提现被支付两次的唯一途径。长时间处于 <code>em_curso</code> 的指令正在核查中：请等待通知。只有在 <code>falha</code> 之后才重发，并使用新的 <code>ref</code>。',
  'dev.api.r4.d': '<code>201</code> 只说明指令已被记录，仅此而已。只有当状态变为 <code>sucesso</code> 时才有结果。在此之前不要把提现当作已完成。',
  'dev.api.motivo': '原因',
  'dev.api.m.naoRecolhida': '十分钟内没有设备取走该指令（无网络、关机）。未执行任何操作',
  'dev.api.m.operadorIndisponivel': '在发送金额之前，运营商回复“服务不可用”。我方每 30 秒重试一次，共 3 次。未执行任何操作',
